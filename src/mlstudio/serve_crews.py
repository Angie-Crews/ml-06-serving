...
Data Source:
    - artifacts/model.joblib (trained in the notebook or app_crews.py)

Terminal commands to run this service from the root project folder:

uv run fastapi dev src/mlstudio/serve_crews.py      # development (auto-reload)
uv run fastapi run src/mlstudio/serve_crews.py      # production

- OR -

uv run uvicorn mlstudio.serve_crews:app --reload    # development (auto-reload)
uv run uvicorn mlstudio.serve_crews:app             # production
...
def load_model() -> RandomForestClassifier:
    ...
    if not MODEL_PATH.exists():
        ...
        raise FileNotFoundError(
            ...
            "Run the training notebook or app_crews.py first."
        )
